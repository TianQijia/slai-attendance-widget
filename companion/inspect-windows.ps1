...
function Get-SlaiQueryIssue($slaiError) {
  if ([string]$slaiError.CategoryInfo.Category -in @("PermissionDenied", "SecurityError")) { return "Denied" }
  if ($slaiError.Exception -is [System.Management.Automation.CommandNotFoundException]) { return "CommandMissing" }
  return "Failed"
}
try {
...
} catch { $slaiResult.profileIssue = Get-SlaiQueryIssue $_ }
try {
  $slaiErrors = @()
  ...
  if ($slaiRules.Count -eq 0) {
    $slaiQueryErrors = @($slaiErrors | Where-Object { [string]$_.CategoryInfo.Category -ne "ObjectNotFound" })
    if ($slaiQueryErrors.Count -eq 0) { $slaiResult.rule = "Missing" }
    else { $slaiResult.ruleIssue = Get-SlaiQueryIssue $slaiQueryErrors[0] }
...
} catch { $slaiResult.ruleIssue = Get-SlaiQueryIssue $_ }
