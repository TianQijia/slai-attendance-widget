...
3. 将信息页中的**手机查看链接**传到自己的手机并打开。链接包含独立查看令牌；本机配对码不要交给手机。再次查看链接可运行 `connection-info` 工具。
...
- 手机每 15 秒检查本地服务，扩展每分钟仅向本机联系一次；学校采集仍为 30 分钟。手机没有学校登录、远程刷新或控制电脑能力。
...
- 查看令牌保存在当前浏览器会话中，链接中的令牌读取后从地址栏移除。重新开标签页可再次使用原链接；普通 HTTP 不保证后台离线页面可用。
