(() => {
  ...
  const failures = {
    ...
    LAN_RESTART_REQUIRED: ["运行中的服务监听地址与当前选择的局域网地址不一致", "重新运行 start 启动器，服务会切换到当前地址；再将新的查看链接传到手机。"],
    CONNECTION_UNCONFIRMED: ["无法核对运行中服务的实例或实际监听地址", "先运行 stop 停止已有服务，再运行新版 start 启动器；若仍失败，运行 diagnose 查看服务状态。"],
