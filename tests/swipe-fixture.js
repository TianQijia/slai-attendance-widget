...
function swipeHtml(day = fixtureDay, { mode = 'normal', pageSize = 10, pageSizeControl = false, emptyCount = true, emptyLabel = '暂无数据', emptyPager = true } = {}) {
  return `<!doctype html><meta charset="utf-8">
    <form id="searchForm"><input name="pageNo" type="hidden"><input name="pageSize" type="hidden"><input name="startTime" value="${day}"><input name="endTime" value="${day}"></form>
    ...
    (() => {
      ...
      function render(data) {
        ...
        document.querySelector('.layui-table-main').innerHTML = '<table class="layui-table">' + rowHtml + '</table>' + (data.total === 0 ? '<div class="layui-none">' + ${JSON.stringify(emptyLabel)} + '</div>' : '');
        ...
        if (data.total === 0 && !${emptyPager}) document.querySelector('.layui-table-page').innerHTML = '';
