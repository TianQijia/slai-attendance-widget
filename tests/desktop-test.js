...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator("#bridgeSettings").getAttribute("open"), null);
    ...
    const before = await page.evaluate(() => [...window.__testMessages]);
    await page.locator("#bridgeSettings > summary").click();
    assert.equal(await page.locator("#bridgeToken").isVisible(), true);
    await page.locator("#bridgeSettings > summary").click();
    assert.deepEqual(await page.evaluate(() => window.__testMessages), before, "Collapsing must not disconnect or re-pair");
    await page.evaluate(() => window.__fixtureNotify({ type: "bridge-state", enabled: true, diagnostic: { code: "BRIDGE_UNREACHABLE", stage: "bridge_push" } }));
    assert.equal(await page.locator("#bridgeSummaryStatus").innerText(), "需处理");
    assert.equal(await page.locator("#bridgeSettings").getAttribute("open"), null);
    await page.evaluate(() => window.__fixtureNotify({ type: "bridge-state", enabled: false, diagnostic: null }));
    ...
    console.log("Passed: calendar/list persistence, date detail keyboard access, one-scroll layout, exact interval/ring totals, collapsed connection, both themes, HiDPI assets and no browser errors.");
